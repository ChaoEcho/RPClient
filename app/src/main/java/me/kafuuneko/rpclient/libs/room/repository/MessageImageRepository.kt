...
class MessageImageRepository(
    ...
    internal suspend fun replaceInTransaction(
        ...
        mImages.insertAll(uuids.mapIndexed { position, uuid ->
            MessageImageEntity(
                key.messageType, key.messageId, position, uuid,
                sendToModel = old.firstOrNull { it.imageUuid == uuid }?.sendToModel ?: true
            )
        })
        mutation.removeFiles((oldUuids - uuids.toSet()).toList())
    }

    /** 生成配图是展示附件，不能覆盖已有上传图，也不能扩大已有消息的视觉输入范围。 */
    internal suspend fun replaceGeneratedInTransaction(
        mutation: FileRepository.Mutation,
        key: MessageKey,
        newUuid: String
    ): Boolean {
        val old = mImages.getByMessage(key.messageType, key.messageId)
        val inputs = old.filter { it.sendToModel }
        if (inputs.size >= MessageImagePolicy.MAX_IMAGES_PER_MESSAGE) return false
        requireNotNull(mFileDao.getByUuid(newUuid)) { "Generated image file is missing" }
        require(mImages.getByImageUuids(listOf(newUuid)).isEmpty()) { "Generated image already has an owner" }
        mImages.deleteByMessage(key.messageType, key.messageId)
        mImages.insertAll(inputs.mapIndexed { position, image -> image.copy(position = position) } +
            MessageImageEntity(key.messageType, key.messageId, inputs.size, newUuid, sendToModel = false))
        mutation.removeFiles(old.filterNot { it.sendToModel }.map { it.imageUuid })
        return true
    }

    /** 正文变化使旧配图失效，但用户上传的附件仍属于这条消息。 */
    internal suspend fun clearGeneratedInTransaction(mutation: FileRepository.Mutation, key: MessageKey) {
        val old = mImages.getByMessage(key.messageType, key.messageId)
        val generated = old.filterNot { it.sendToModel }
        if (generated.isEmpty()) return
        mImages.deleteByMessage(key.messageType, key.messageId)
        mImages.insertAll(old.filter { it.sendToModel }.mapIndexed { position, image -> image.copy(position = position) })
        mutation.removeFiles(generated.map { it.imageUuid })
    }
