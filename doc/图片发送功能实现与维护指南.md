...
```json
{
  ...
  "extra": {
    "rpclient": {
      "schema_version": 1,
      "images": [
        { "mime_type": "image/png", "data": "完整图片字节的 Base64" }
      ]
    }
  }
}
...
| 字段 | 约束与含义 |
| --- | --- |
| `extra.rpclient.schema_version` | 图片扩展版本，目前为 `1`；有 images 但版本不支持时拒绝导入 |
| `extra.rpclient.images` | 有序图片数组；空数组明确表示没有图片 |
| `mime_type` | 文件格式提示；入库和导出根据真实字节校验，不只相信归档声明 |
| `data` | 无 Data URL 前缀、无换行的标准 Base64；保存当前历史附件的实际字节 |
...
flowchart TD
    ...
    C --> D{附件文件存在?}
    D -->|是| E[校验实际格式并流式编码 Base64]
    D -->|否| F[跳过并累计数量]
    E --> G[写入 extra.rpclient.images]
    F --> G
    G --> H[完成导出并提示缺图数量]
```
...
`ChatArchiveCodec` 写出常规消息字段，`ChatArchiveImageCodec.writeImage()` 从文件读取字节并分块写出 Base64。导出不会先拼出完整图片字符串，也不会把完整归档装进内存。Base64 包装流关闭时只完成当前图片尾块，不关闭外层 JSONL Writer。

快照在整个导出期间持有文件锁和数据库事务，代价是较大归档会延后其他需要这些锁的文件修改。维护时不能直接将它改为“先开 Room 事务，再调用 withFileLease”。

导出保存的是历史中已经存在的最终图片，不会恢复首次选图前的未压缩原图，也不按当前压缩设置重新编码。图片文件存在但实际内容损坏时导出失败；仅文件已丢失的情况跳过并提示。导出失败可能在目标文档留下不完整内容，需要重新导出。
...
flowchart TD
    ...
    B --> C{图片来源}
    C -->|RPClient Base64 或 Data URL| D[解码到本次导入的私有暂存]
    C -->|酒馆路径引用| E[可选：系统选择图片目录]
    E --> F[按完整相对路径或路径后缀匹配]
    D --> G[确认绑定本地角色]
    F --> G
    G --> H[恢复图片并创建 PreparedFile]
...
`ChatArchiveCodec` 使用字符流解析，RPClient 自有数组每解析一张图片就调用暂存回调，避免同一消息的所有 Base64 同时进入 JSON 树。单张 Base64 仍会短暂驻留字符串；旧酒馆字段先解析单条消息再提取图片，因此不是所有输入形式都完全常量内存。
...
确认后 `MessageImageRuntime.prepareArchive()` 保留原始归档字节，校验真实格式和尺寸，为 `FileRepository` 创建 `PreparedFile`。全部图片在事务外准备完成，再通过同一个 `FileRepository.mutate()` 原子提交：文件索引、会话、消息及 `MessageImageEntity`。新附件重新分配 UUID，继续使用现有 hash 去重和引用回收机制。
...
| 自有内嵌 Base64 损坏、图片无效或扩展版本不支持 | 整体导入失败，不伪装成一次完整成功的图片备份恢复 |
| 角色被删除、文件提交或数据库写入失败 | 回滚事务并释放本次准备的文件；不留下半个新会话 |
| 用户取消或任务取消 | 关闭输入输出流，清理暂存和未提交凭据 |

导入文件总量上限为 **512 MiB**，单图原始字节上限沿用 **32 MiB**，单条消息归档图片上限为 **256 张**；目录扫描限制深度和条目数量。Base64 通常比原始字节增加约三分之一。超过导入上限的巨大导出文件需要后续另行设计分卷，当前不能承诺任意大小归档都能重新导入。
...
本次离线验证通过：聊天相关单元测试 15 项，Debug / androidTest 构建成功；独立模拟器上的归档与图片生命周期仪器测试 26 项通过。酒馆兼容结论依据上述固定版本源码和协议测试，未运行酒馆浏览器界面端到端导入。
