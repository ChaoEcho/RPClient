# RPClient 故事编写功能完整实现方案（旧版连续正文设计）

> 本文保留为 V1 设计记录。当前分卷/章节结构、数据模型、并发策略与归档格式以
> [小说分卷章节结构化方案](./小说分卷章节结构化方案.md) 为准。
...
- 为当前 Story 关联 0～N 张角色卡，配置主角/常驻/自动模式，并可设置至多一个主角。
- 选择当前 Story 启用的世界书条目；条目的角色作用域由角色卡关联的世界书自动推断。
- 可选注入当前全局用户人设描述。
- 文档末尾续写。
...
flowchart LR
    ...
    NR --> DB["Room: stories + story_characters + story_lorebook_entries"]
...
V1 使用一个正文实体、一个角色关联实体和一个世界书条目关联实体。
...
- `includeUserPersona`：是否把当前全局用户人设描述作为独立系统参考注入写作 Prompt。
...
- `ACTIVATION_PRIMARY` 表示唯一主角，无条件注入，并作为无角色关联世界书的宏作用域。
- `ACTIVATION_ALWAYS` 用于普通常驻人物。
- `ACTIVATION_AUTO` 根据当前操作附近正文中的角色卡名称动态激活，适合配角。
- 角色卡名称是角色身份和 `{{char}}` 的唯一事实来源；故事内称谓、昵称和关系写入 Memory。
- Primary、Always 和 Auto 是互斥激活状态；Repository 校验 Primary 在当前 Story 内至多一个。

### 5.3 StoryLorebookEntry

`story_lorebook_entries` 使用 `(storyId, lorebookEntryId)` 作为联合主键，保存
`activatedAtStep`、`stickyUntilStep`、`cooldownUntilStep` 和 `stateSignature`。
删除 Story 或原始条目时级联删除关联。

世界书宏的解析优先级为：按 Story 角色顺序查找 `characterLorebookId` 命中条目所属世界书的
首个角色 → Story 主角 → 保留 `{{char}}` 原文。多个角色关联同一本世界书时使用 Story
角色顺序中的第一个。
`{{user}}` 始终替换为当前用户名。条目内容、主关键词和次要关键词必须在激活扫描前使用
完全相同的作用域展开。

## 6. 数据库升级

当前开发版本允许破坏性调整，因此本轮直接重写 version 3 的最终 schema，不增加数据库版本号。
version 3 包含 `Story`、`StoryCharacter` 和 `StoryLorebookEntry`：
...
libs/room/entity/StoryLorebookEntry.kt
libs/room/dao/StoryDao.kt
libs/room/dao/StoryCharacterDao.kt
libs/room/dao/StoryLorebookEntryDao.kt
...
suspend fun getStoryLorebookEntryCandidates(
    storyId: Long
): List<StoryLorebookEntryCandidate>
...
`updateStoryConfiguration` 在一个事务中校验并替换角色与世界书关联，不能留下半更新状态。
...
Repository 读取候选时按 `sortOrder` 排序；角色卡名称始终直接来自角色卡实体。
...
V1 将 Story 列表直接整合到主页，并使用独立创建页完成名称、角色卡和世界书配置：

```text
feature/main/
├── presentation/               # 首页 Story 列表与多选状态
├── model/                      # 首页 Story 列表项和稳定选择键
└── ui/MainLayout.kt            # Story 卡片、筛选和批量删除交互

feature/story/
├── create/
│   ├── StoryCreateActivity.kt
│   ├── StoryCreateViewModel.kt
│   ├── presentation/
│   ├── model/
│   └── ui/StoryCreateLayout.kt
└── editor/
...
首页的“故事创作”入口直接打开 Story 创建页；已有 Story 在主页与单聊、群聊会话一同管理。

主页 Story 列表提供：

- 标题、字符数、短预览和更新时间。
- 点击 Story 直接打开编辑器。
- 长按进入主页多选模式，并支持与其他首页内容一致的批量删除确认。
- 卡片菜单支持重命名；空列表提供创建引导。
...
- 设置 Story 名称。
- 多选本次 Story 使用的角色卡。
- 按世界书分组选择，并可单独开启或关闭世界书条目。
- 创建成功后直接打开连续正文编辑器；创建与关联配置作为一个完整业务操作提交。
...
- 添加和移除候选角色卡。
- 在 Primary、Always 和 Auto 三种互斥激活模式之间切换。
- 调整稳定注入顺序。
...
同一角色只出现一次，原因优先级为 `Primary > Always > Name`，`matchedKey` 仅用于人类可读的检查信息。
...
选择 Story 角色时，角色的 `characterLorebookId` 会把对应世界书中尚未启用的条目默认加入
`StoryLorebookEntry`。用户之后仍可按整本或单条关闭，持久化关联表是重新加载设置和构建
Prompt 时的唯一启用依据。

### 11.7 世界书

世界书候选条目来自 `StoryLorebookEntry`。角色卡通过 `characterLorebookId` 关联的世界书在
选择角色时提供默认勾选，并在 Prompt 构建时提供 `{{char}}` 作用域，但不绕过 Story 的
世界书开关。
...
- 哪些正文段落进入了上下文。
- 哪些角色卡因 Primary、Always 或名称命中被激活，以及哪些允许字段进入了上下文。
- 哪些世界书被激活和保留。
...
- 角色卡使用名称和内容 fingerprint 提示匹配；Hint 直接保存 `primary/always/auto` 和数组顺序。
...
app/src/main/java/me/kafuuneko/rpclient/feature/story/create/
...
- Always 角色始终激活；Auto 角色只由角色卡名称激活。
...
- Story 角色关联替换具有事务性，并完整保存顺序、模式和唯一主角。
- Story 世界书关联完整保存启用条目和条目级 timed state；角色作用域变化由解析后条目签名使旧状态失效。
...
- 新建 Story 后直接进入编辑器。
- 选择、取消、重排角色卡以及修改模式/主角后，下一次 Prompt 使用最新配置。
- 返回和进入后台前正文完成保存。
...
4. 用户可以为 Story 关联多张角色卡，并分别配置 Primary、Always、Auto 和顺序。
5. 一轮可以同时激活多张角色卡；Prompt 只注入 Always 和本轮角色卡名称命中的 Auto 角色，并保持稳定顺序。
