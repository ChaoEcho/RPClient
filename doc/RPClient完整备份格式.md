...
集成分支写入 `backupVersion = 2`，读取版本 1 / 2。外层加密容器仍为版本 1，密码派生与认证加密方式不变。以下 V1 契约保留为历史兼容定义；V2 在此基础上增加：

- 必需表 `tables/message_images.jsonl` 与 `tables/llm_token_usage_records.jsonl`，即使为空也必须出现并登记清单计数。
- 附件的 `sendToModel` 是必须显式提供的布尔值。普通上传图为 `true`，生成配图为 `false`；缺失时拒绝恢复，不擅自扩大模型可见范围。
- 恢复后校验附件的单聊/群聊父消息和文件引用；用途标记随备份保留。
- V1 行中缺失的模型能力、并发和外貌缓存字段使用对应历史默认值。旧 `imageFileUuid` 转换成展示附件，不能以曾固定为 5 的 `databaseVersion` 判断格式。
- 偏好快照继续使用自己的版本 1，新增可选主题与图片发送设置；历史快照缺失时恢复为系统主题与自动图片设置。
- 完整恢复使用应用私有回滚日志，先阻止旧任务写回，再执行替换；故障边界和验证命令见 `build-and-verification.md`。

V2 是候选集成格式；发布前必须通过设备上的 V1/V2 恢复、Room 迁移和 R8 回归。
