...
角色范围的脚本以 Room 为权威存储；导入角色卡时会从
`extensions.regex_scripts` 提取，导出时再写回该兼容字段。在群聊中，系统会收集群内各角色已授权的脚本。
...
3. 确认当前选中的 Global 或 Character 范围正确。
4. 在测试区选择 Placement。
...
测试页用于验证当前范围内脚本的文本变换，不要求 Character 已授权。

真实聊天还需要满足：

- Character 范围已开启“允许脚本执行”。
- 当前会话使用了对应角色。
- Placement 与真实文本类型一致。
- 模式与实际执行阶段一致。
...
- Global 与 Character 脚本统一保存在 Room；导出角色卡时会重新写入
  `extensions.regex_scripts`，导入时则从该兼容字段提取到 Room。
- 导入角色脚本后仍需自行确认 Character 的执行授权。
